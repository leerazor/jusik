...
자동 연구는 함께 추적하는 `research-mandate.json`의 최신 사용자 의사를 따릅니다. 현재 JSON SHA-256은 `abbd7cf0b93bbdb995e15315ea6b93bc7526e420a51214515056bb68ccf5c2ff`입니다. 과거 실험이 고정한 원본과 계약은 해당 실험의 근거로 보존하며, 새 조건을 적용했다고 과거 결과를 다시 해석하지 않습니다.
...
- 무료 근사 자료는 날짜별 표본으로 개인 판단에 참고할 수 있지만 strict PIT 검증·완전한 생존자/상장폐지·배당 자료로 표시하지 않습니다. KRX/공공 목록, Alpha Vantage 상장 상태, Yahoo 과거 일봉, FRED DEXKOUS를 CLI `collect`의 bounded network cache로 수집할 수 있으며 현재 후보를 과거에 소급하지 않습니다. 웹은 준비된 cache만 읽고, 키·자료가 없거나 일부 응답이 실패하면 명시적으로 확인 불가로 남깁니다.
