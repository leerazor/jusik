...
- 상태: 조사. 이전 초보자 UI에 대한 사용자 피드백을 반영합니다. runner는 시작 당시 unpaused·service inactive·timer active·running attempt 0이었으며 수동 수정 중에만 pause·service 중지했습니다.
...
- 작업 경로·브랜치: 계획 확정 후 `/home/kwl/projects/jusik-research-web-reports`, `feat/research-web-reports` 생성 예정. 조사 기준 `a4194781be2655c398d85e493622626781bfc269`; 통합 대상 local `main`.
- 허용 범위: frontend 연구·보고서 읽기와 표현, 필요한 검증된 보고서 조회 경계와 집중 테스트, 관련 사용자 문서·영구 기록. 전략·금융 계산·catalog 결과·인증·주문·운영 데이터 변경 제외.
...
- worktree: `/home/kwl/projects/jusik-lab-continuous-research-dispatch`; branch `feat/lab-continuous-research-dispatch`, 기준 `aee6516`. 통합 검증·산출물 보존 후 전용 worktree/venv/cache 정상 제거, source branch/commit 보존.
- 검증: 연구 선순위·PASS 전 enqueue 0·독립 PASS 후 1건·대기/REJECT 후 공학 계속·동일 identity 중복 금지·재시작/기한/pause/queue cap·DB-bound scope와 followup 우회 차단·불확실 orphan 영속 hold를 검증했다. 운영 backup 복사본의 기존 10개 테이블 1,039개 행 보존·반복 초기화·integrity PASS.
- 남은 경계: 새 연구 완료의 독립 reviewer 자동 연결, reviewer transient retry 확장, no_work 증거 강화는 후속이다. 전체 투자 검증 pipeline 완료나 새 수익성 성과로 표시하지 않는다. 개발 기록: `docs/development-records/2026-09-26-lab-continuous-research-dispatch.md`; handoff: `docs/handoffs/2026-09-26-continuous-research-dispatch.md`.
...
- 운영: 기록 저장 중 pause, timer 활성 유지. 기록 commit 뒤 tracked-clean main에서 자동실행을 재개한다. 사용자 미추적 `HANDOFF.md`, 이전 작업·실패 기록 보존. 단일 Sol 구현 소유자와 별도 Sol reviewer, 동시 active 최대 4명.
...
- 기록: `docs/development-records/2026-09-26-lab-engineering-discovery.md`; handoff: `docs/handoffs/2026-09-26-engineering-discovery.md`. 운영 후에도 기존 task 140개·attempt 199개·review 5개의 행이 사전 백업과 동일하며 integrity `ok`다. 완료 worktree는 정상 제거했고 코드·검증·설치 캐시는 보존했다. 자동 재개 확인은 기록에 연결한 외부 `RUNTIME.md`와 현재 DB를 대조한다.

## lab-discovery-c478225b8a11df4e9207f443acec017d

- 상태: 완료. 첫 자동 발굴·범위 검토 PASS로 등록했으며 환경 복구 후 새 attempt의 구현·별도 review PASS로 `ENGINEERING_COMPLETE/NOT_EVALUATED`다.
...
- 보존 상태: 제품 commit `548cdd5`를 main `ec77cd4`로 적용했고, 동일 제품 내용 확인 후 `136f050`으로 원래 branch 이력도 연결했다. 첫 attempt `24fb6de0a54a40b2a12771ad30f07b7e`는 FAILED 그대로 보존한다. 실패 출력을 다시 쓰지 않는다.
...
- 검증: 새 attempt `6aa4afdbd9054eaeada5e8e63971f915`, 별도 review `27490b947c8c48ae8b66e1b9151f1370` PASS. main 제품 pytest 13개, Ruff check/format, strict mypy 통과. 설치 cache를 audit에 옮기고 완료 worktree는 정상 제거했다. 관련 기록은 `docs/development-records/2026-09-26-lab-engineering-discovery.md`다.
...
- 상태: 완료. 복구 코드 local `main` 통합·독립 검토 PASS, 운영 복구 후보와 별도 reviewer PASS 확인.
...
- 상태: 완료 (공학만). spec과 제품 코드 local `main` 통합, 제품 독립 검토 및 운영 복구 reviewer PASS. 투자 검증은 `NOT_EVALUATED`.
...
- 상태: 코드 local `main` 통합·독립 검토·통합 검증 완료; 운영 자동 실행 확인 중. 수동 개발 동안 roadmap runner는 pause, service inactive; timer는 유지했다.
...
- 워크트리: `/home/kwl/projects/jusik-lab-continuous-engineering-backlog`; 브랜치 `feat/lab-continuous-engineering-backlog`; 기준 `626303f`, 최종 구현·local main 통합 `c62d512`.
...
- 검토·검사: 독립 Sol review의 두 P2(READY 복귀 stale idle, 소진 기록 경합)를 수정·재검토 PASS. 통합 후 runner pytest 161개, Ruff check/format, strict mypy 4개 파일 통과. 운영 설정은 원본을 private audit에 백업하고 `automatic_engineering_backlog=true`만 추가했으며 실행기 재개·실제 child 결과는 별도 확인한다.
...
- 워크트리: `/home/kwl/projects/jusik-lab-independent-review-receipt`; 브랜치 `feat/lab-independent-review-receipt`; 기준 `3a9ec25`, 최종 구현 `f954568`, local main 통합 `f954568`.
...
- 검증: 후보/검토 attempt 분리, PASS identity/hash/current HEAD 재검증, timeout·중단 복구, 원자 완료, BLOCKED/WAITING 뒤 독립 READY, 기존 연구 호환을 임시 저장소·SQLite로 검증했다. worker focused 145 passed, 독립 Sol 재검토 PASS, main 통합 후 focused 145 passed·Ruff·strict mypy·diff check 통과.
...
- 상태: 설계와 최소 구현을 local `main`에 통합·검증. 독립 reviewer dispatch/receipt는 위 별도 작업에서 통합. 기존 차단 attempt는 소급 완료하지 않는다.
...
- 2026-09-24 request identity 보강: `CacheEntry.request_descriptor`를 추가해 API secret을 제거한 canonical source/URL/method/params/data를 새 manifest에 보존했습니다. fresh 137 entries 모두 descriptor가 있고 focused collector pytest 146, Ruff check, strict mypy가 통과했습니다. coverage는 여전히 `27472/20306`으로 R1-05 checkbox는 미체크입니다. 기록 `docs/development-records/2026-09-24-r1-05-request-descriptor.md`, manifest `/home/kwl/.local/share/jusik/portfolio-audit/20260924-r1-05-receipt/fresh-cache-0945/manifest.json`.
- 2026-09-24 fresh provider receipt: 새 cache에서 US 1년/100심볼 bounded collection을 수행해 Alpha Vantage 2건, Yahoo 76건, FRED 59건의 raw/checkpoint receipt를 보존했습니다. expected/actual/retained 세션은 `27472/20306/20306`, missing `7166`으로 coverage가 불완전해 R1-05 checkbox와 경제 acceptance는 미승격합니다. 기록 `docs/development-records/2026-09-24-r1-05-fresh-provider-receipt.md`, cache `/home/kwl/.local/share/jusik/portfolio-audit/20260924-r1-05-receipt/fresh-cache-0941/`.
- 2026-09-24 exclusion coverage correction: 명시적 request-excluded 25개 심볼의 6,800개 세션을 eligible missing에서 분리했습니다. 비제외 실제 누락은 LIME 221개·MDA 145개로 남아 있으며 collector pytest 146·Ruff·strict mypy를 통과했습니다. R1-05 checkbox/PIT/economic acceptance는 유지합니다. 기록 `docs/development-records/2026-09-24-r1-05-exclusion-coverage.md`.
- fresh result 재생성: 기존 `fresh-cache-0945`를 수정 collector로 재사용해 `fresh-result-1010.json`(SHA `b659c38676d531b70c3ea172fdcb5e2680bd4eb0ec6a1f2a2f9f913ba50c5d0e`)을 생성했습니다. eligible coverage `20672/20306`, missing `366`(LIME/MDA)이며 network 0회, R1-05 승격 없음.
- partial-history 근거 보강: `CollectionSymbolDiagnostic.first_observed_session`을 추가해 provider 응답에서 실제 관측된 첫 세션을 보존합니다. LIME 2026-07-01, MDA 2026-03-12의 시작일은 historical observed_at·원인 증거가 아니므로 366개 누락과 R1-05/PIT/economic acceptance 차단을 유지합니다. collector focused 회귀·strict mypy 통과.
- R2-01 명시적 retry `d30192ee317c4995bb84bcb6fba8c7c7`는 10:47:25 KST에 시작했으나 약 3분 동안 completion JSON 없이 review dispatch 준비 로그만 남아 서비스 정지로 `interrupted` 복구했습니다. 90분/15분 timeout 초과를 주장하지 않으며, 성공·review PASS·경제 acceptance로 해석하지 않습니다. child dispatch 원인 확인 전 반복 retry하지 않습니다. 기록 `docs/development-records/2026-09-24-r2-01-timeout-retry.md`.
- R1-05 Alpha fallback receipt: LIME/MDA에 Alpha Vantage `TIME_SERIES_DAILY`를 bounded 요청했으나 free key의 `outputsize=full` premium 제한 안내만 반환되어 두 심볼 모두 0행입니다. raw SHA `37aaccd80b9e6babd46c5275fa4692477b19eb2354b0773e78c8c473b7a87e5f`, manifest SHA `9a503b14902fd54cda02e4897308a2625c59e01482a609a6e2033ffc8be8d6f7`. 366개 Yahoo 누락과 R1-05/PIT/economic acceptance는 유지합니다. 기록 `docs/development-records/2026-09-24-r1-05-alpha-bounded-receipt.md`.
- R1-05 Yahoo 장기 fallback: `period1=0`으로 LIME/MDA를 재요청해도 각각 첫 관측일 `2026-07-01`/`2026-03-12`로 변하지 않았고 `firstTradeDateMilliseconds`는 null이었습니다. raw SHA LIME `a71df622f096623fba9e798e32020cddde5f3e6455bc8f440450f0af4e93238f`, MDA `2d390678f3972560eb0ab8f844953d67ec9a2902564386ae747cdec84b740493`, manifest SHA `1718cda518b062d625a1e1a058104cab3f2cc61176a490e68a972ba314af9f8e`. 원인/observed_at 부재로 366개와 acceptance는 유지합니다. 기록 `docs/development-records/2026-09-24-r1-05-yahoo-long-history.md`.
- 후속 guard: runtime prompt에 receiver가 없는 turn에서 `collaboration.spawn_agent`를 호출하지 않고 현재 supervisor의 bounded review 또는 finite `review-unavailable`로 종료하도록 명시했습니다. development runner focused pytest 67개·Ruff·strict mypy 통과. 기록 `docs/development-records/2026-09-24-runner-empty-review-dispatch.md`.
- runner child가 receiver capability를 추정해 spawn하지 않도록 prompt를 엄격히 고정했습니다. child는 자체 bounded review 또는 finite unavailable evidence만 반환해야 하며, focused pytest 67·Ruff·strict mypy를 재확인했습니다.
- R2-06 blocked completion에서 자료 부족 followup을 함께 반환해 `completion_invalid`가 된 계약 오류를 보강했습니다. blocked 결과는 `followup=null`이어야 한다는 prompt guard를 추가했고 runner pytest 67·Ruff·strict mypy를 재확인했습니다.
- fresh R2-01 retry `d554a02dec5749b796134dfa5f9ee436`에서 configured mypy test typing 오류 뒤 child의 빈 receiver `collaboration.wait`를 guard가 감지해 `failed`/`empty_review_dispatch`로 유한 종료했습니다. 무기한 대기·completion·review PASS·통합 성공은 없으며 기록 `docs/development-records/2026-09-24-r2-01-empty-review-dispatch.md`에 보존합니다.
- mypy 경계 수정: float 거부 회귀의 실제 float 값을 유지하면서 `cast(Decimal, 0.1)`로 정적 타입 오류를 제거했습니다. accounting pytest 39·Ruff check/format·strict mypy 통과. 독립 review와 full R2-01 경제 acceptance는 아직 미완료입니다.
- 독립 review P2 수정: FX 관측 입력이 없는 현재 API에서 USD 회계가 `complete`가 되지 않도록 status gate를 보강하고 회귀를 추가했습니다. KRW native 완료 의미는 유지하며 accounting pytest 40·Ruff check/format·strict mypy 통과. 실제 FX·완전 회계 자료와 full R2-01 acceptance는 여전히 미완료입니다.
...
- 재시도 진행 (`5afa52b443fd48f281a9a1f623d100a8`): 승인 main `bf7905e9fd7daab363d7cdc56114ed278965f4ea` 및 입력6개 SHA/크기 일치. 기존 소유 `/home/kwl/projects/jusik-r1-receipts-621e0204`, `docs/r1-receipts-621e0204`를 재사용합니다. 소유 루트 `.venv` Python3.13.15 확인. Luna 조사 후 한 구현 소유자가 기존 감사 helper로 새 원문 대사·결정성·focused pytest/Ruff/mypy를 실행하고 Terra 독립 review, Astra local main 통합 검사를 수행합니다. 계획·증거: `/home/kwl/.local/share/jusik/portfolio-audit/20260923-r1-05-5afa52b4/PLAN.md`. CPU900초 마감22:58:01Z, 입력32MiB/raw137/진단101/신규20MiB를 유지합니다. 전체 R1-05/PIT/경제 acceptance는 미승격하며 다른 task와 사용자 HANDOFF를 보존합니다.

- 최신 시도 중단 (2026-09-23 KST): 이번 시도 `f3d2d034e7fd4cbd943b60eb304af79f`는 세션 마감 `2026-09-22T23:59:59+09:00`이 지난 뒤 시작되어 중단했습니다. 최초 clock 관측은 `2026-09-22 22:38:39 UTC`이며 승인 baseline `1fedd08e7e432f161c27cc3d6d5ac20a271aa891`과 실제 main은 일치합니다. R0 다섯 항목 완료와 R1-05 미체크를 확인했습니다. 원문 SHA/size·checkpoint 대사·환경 복구·테스트·독립 review·구현 통합은 실행하지 않았으며 이전 PASS를 재사용하지 않습니다. 소유 worktree `/home/kwl/projects/jusik-r1-receipts-621e0204`는 clean, HEAD `b4692e5e9c96fa1362d472fd9d8dc98fcd4845ec`로 보존합니다. 다른 작업과 기존 untracked `HANDOFF.md`를 보존했습니다. 네트워크·simulation·GPU·주문·PAPER/live·DB·서비스·설정·remote 변경은 없습니다. 전체 coverage/PIT·R1-05·경제 acceptance는 미승격입니다. 새 마감 시각을 승인받은 뒤 동일 branch에서 소유 Python3.13 환경을 먼저 확인하고 고정 입력 검증부터 재개해야 합니다. 외부 provider receipt에는 심볼·기간·거래소·통화·request/session identity·cause·historical observed_at·전체 coverage가 필요하며 captured_at으로 대체하지 않습니다. 증거와 인계: `/home/kwl/.local/share/jusik/portfolio-audit/20260923-r1-05-f3d2d034`. 이번 중단은 자동 복구 label에 해당하지 않습니다.

- 최신 시도 차단 (2026-09-22, `621e02047dc24fa0bc393969f86d5d1c`): 승인 baseline·입력6개 SHA는 일치했으나 소유 환경이 Python3.12.3(요구3.13)이며 mypy 실행 파일이 없어 `dependency_setup` gate에서 중단했습니다. 초기 다른 환경의 PASS는 완료 근거에서 제외합니다. fresh 감사 초안은 보존하지만 required tests·최종 독립 review·구현 main 통합은 미완료입니다. worktree `/home/kwl/projects/jusik-r1-receipts-621e0204`, branch `docs/r1-receipts-621e0204`를 보존합니다. audit·인계: `/home/kwl/.local/share/jusik/portfolio-audit/20260922-r1-05-621e0204`. 다음 재시도는 같은 branch에서 소유 Python3.13 의존성 setup을 먼저 검증합니다. 전체 R1-05/PIT·경제 acceptance는 미승격이며 다른 task는 재개하지 않았습니다.

- 재시도 진행 (2026-09-22, `621e02047dc24fa0bc393969f86d5d1c`): 새 승인 main `72275b2bfe1316d9015a843075f8b44afc19ab7e`와 입력 6개 SHA가 일치합니다. R0 완료·R1-05 미체크를 유지합니다. 기존 소유 worktree는 정리되어 있고 archive branch만 남아 있으므로 새 전용 worktree `/home/kwl/projects/jusik-r1-receipts-621e0204`, branch `docs/r1-receipts-621e0204`에 Luna 한 명을 배정합니다. 수정은 같은 개발 기록과 별도 audit에 한정하며 collector·원문·다른 task·사용자 HANDOFF를 보존합니다. 기존 helper의 fresh 원문 대사·결정성·focused pytest/Ruff/configured mypy, Terra 독립 review, Astra local main 통합 검증을 수행합니다. CPU 900초(12:08:48Z~12:23:48Z), 입력32MiB, raw137/진단101, 신규 artifact20MiB; 네트워크·simulation·GPU·PAPER/live·DB·서비스·설정·remote 변경0. 현재 runner child만 running이며 runner/service를 변경하지 않습니다. audit·handoff: `/home/kwl/.local/share/jusik/portfolio-audit/20260922-r1-05-621e0204`. 과거 중단과 예산·정리 순서 위반은 소급 승인하지 않습니다.

- 최신 시도 차단 (2026-09-22, `13d842d0596243f9ab1ef20998c2255f`): 요청 시작 main `31ed0ad204992a64316f4afc8c8fc1d7b3bd8b58`와 관측 `7f211de04641bb760c79265d6071245fc8ae5781`이 다릅니다. 첨부 입력 5개 SHA는 일치하며 R0 완료/R1-05 미체크를 확인했습니다. 시작 identity gate에서 중단하여 새 구현·원문 대사·테스트·독립 review·통합은 수행하지 않았습니다. 이전 attempt의 예산 초과와 정리 순서 위반은 역사로 보존합니다. 기존 소유 worktree는 이미 정리되어 있고 다른 task는 재개하지 않았습니다. 현재 main과 결속한 승인 입력이 필요하며 자동 복구 사유는 지정하지 않습니다. 증거·인계: `/home/kwl/.local/share/jusik/portfolio-audit/20260922-r1-05-13d842d0`.
...
- 보조 조사 (2026-09-22): Alpha Vantage `LISTING_STATUS(date=2025-09-11)` 1회(HTTP 200, raw 867,056 bytes)에서 제외 25개 중 24개가 원문에 존재하고 모두 `Active`, `DYNX`만 미일치였다. 이를 Yahoo 실패 원인·거래중단·PIT publication으로 추정하지 않는다. receipt `/home/kwl/.local/share/jusik/portfolio-audit/20260922-alpha-excluded-listing-20260922/receipt.json`, 기록 `docs/development-records/2026-09-22-alpha-excluded-listing-receipt.md`.
- 보조 조사 (2026-09-22): 제외 25개 Yahoo를 동일 기간으로 1회 재조회했다. HTTP 404 20개(`No data found, symbol may be delisted` 원문 보존), HTTP 200 5개 중 3개 272세션·2개 timestamp 0개였다. raw 25개/92,284 bytes의 URL·기간·상태·SHA는 `/home/kwl/.local/share/jusik/portfolio-audit/20260922-yahoo-excluded-receipts-20260922/receipt.json`에 보존했다. 404 문구를 historical delisting 원인으로 승격하지 않으며 PIT·경제 acceptance는 미승격한다. 기록: `docs/development-records/2026-09-22-yahoo-excluded-receipts.md`.
- 보조 조사 (2026-09-22): Yahoo search로 404 심볼 20개의 alias 후보를 조회했으나 동일한 US equity identity는 확인되지 않았다. 타 거래소·fund·무관 후보는 적용하지 않았다. raw search receipt는 `/home/kwl/.local/share/jusik/portfolio-audit/20260922-yahoo-symbol-search-20260922/receipt.json`, 기록은 `docs/development-records/2026-09-22-yahoo-symbol-search.md`이다.
...
- 상태: 완료·새 PAPER 계약 대기
- BanKIS online KRX/NXT/US 수수료·세금 profile과 deterministic hash-bound PAPER cost contract/manifest validator를 추가했습니다. `applies_to_frozen_history=false`와 hash mismatch 검증으로 기존 historical modeled rate와 결과를 보호합니다.
- 검증: cost profile `4 passed`, Ruff, strict mypy, diff check 통과.
...
- 상태: 완료
- RSS/news category를 `NewsCategory` literal과 typed parser tuple로 고정했습니다.
- 검증: news strict mypy·Ruff 통과, 관련 테스트 `9 passed`
- 개발 기록: `docs/development-records/2026-09-20-news-category-strict-typing.md`
...
- 상태: 완료
- R5 후보 생성기의 method/gate 목록을 typed literal tuple로 고정했습니다. 후보 수·순서와
  자동 선택 정책은 변경하지 않았습니다.
- 검증: portfolio engine strict mypy·Ruff 통과, 관련 테스트 `59 passed`
- 개발 기록: `docs/development-records/2026-09-20-portfolio-candidate-strict-typing.md`
...
- 검증: `test_research_engine.py` 14 passed 및 인접 회계/근사자료 회귀 78 passed, Ruff,
  `git diff --check`; strict mypy는 기존 인접 모듈 오류 5개로 실패(변경 파일 신규 오류 없음)
...
- public evidence catalog의 SHA·item count·source count를 coverage 계산 전에 재검증하도록
  ...
  catalog writer도 같은 검증을 거친 뒤에만 파일을 저장합니다.
...
- `verify_sec_review_queue_sources()`를 추가해 queue accession별 bounded raw HTML 존재와
  ...
  확인했습니다. manifest 생성 경로도 evidence 파일 존재·크기·SHA를 재검증하며,
  `research_sec_evidence --verify-queue` CLI로 같은 gate를 재현할 수 있습니다.
...
- `validate_mandate` digest `22efba...c264ab1`, roadmap 40개/complete 16개와 현재 roadmap digest
  `17a93d00fda091024aafb44b3661942c3257e5127b040d868934eb910f1e9048`를 재확인했습니다.
...
- 최신 main 검증: counterfactual/accounting focused pytest 79개, 포맷 보정 후 counterfactual pytest 42개·Ruff·strict mypy 통과. 독립 review dispatch의 receiver 없는 wait는 반복되어 interrupted로 기록하며 성공 승격하지 않습니다.
- runner 추가 보호: child stdout에서 빈 `receiver_thread_ids`의 collaboration wait를 감지해 `empty_review_dispatch`로 유한 실패 처리하도록 보완했습니다. runner focused pytest 67개·Ruff·strict mypy 통과. review 성공·R2-06 checkbox·경제 acceptance는 자동 승격하지 않습니다.
- 2026-09-24 재시도 `86c8448a30e646b993c1fcb430b173ea`는 main 통합 상태의 R2-06 기술 검증을 재확인했습니다. focused pytest 78개, Ruff check/format, strict mypy와 독립 검토에서 숫자 경계 결함 해소를 확인했으나 runner의 review dispatch가 수신 agent 없이 대기해 fail-closed 중단했습니다. 기술 모듈·CLI는 main에 통합·검증됐지만 R2-06 checkbox와 경제 acceptance는 유지합니다. 기록 `docs/development-records/2026-09-24-r2-counterfactual-86c8448a.md`, audit `/home/kwl/.local/share/jusik/portfolio-audit/20260924-r2-06-86c8448a/`.
- 2026-09-24 재시도 `968bb194e89643e9a6b4060e3ce17fec` 차단: 확인된 최신 main `d64e82a134e191349b4989c4fe230bcdc4bf11fe`는 원 요청 기준 `d0d0299`와 이전 기준 `450bf98`의 자손입니다. 기존 소유 worktree/branch와 미병합 `ba0a10e`를 재사용 대상으로 확인했습니다. 승인된 세션 마감 2026-09-24 01:09:53 KST가 이미 지난 08:59 KST에 확인되어 배정 직후 작업자를 중단했습니다. 새 구현·검사·독립 검토·main 통합은 수행하지 않았습니다. worktree는 변경 없이 보존하고 R2-06 checkbox는 유지합니다. 재개에는 새 세션 승인과 공개 mapping 숫자 경계 수정, fresh 검사·독립 검토가 필요합니다. 전체 자료 acceptance에는 완전한 준비 결과·benchmark·미래 관찰이 별도로 필요합니다. 기록 `docs/development-records/2026-09-24-r2-counterfactual-968bb194.md`, audit `/home/kwl/.local/share/jusik/portfolio-audit/20260924-r2-06-968bb194/`.
- 2026-09-23 재시도 `2817a7d9b9274b7ea6fa3810a98ed32e` 차단: 기준 main `450bf982e52663c0cad641fd43e7442c4d463ea2`와 이전 입력 SHA를 확인했습니다. 소유 worktree `/home/kwl/projects/jusik-r2-counterfactual-2817a7d9`, branch `feat/r2-counterfactual-2817a7d9`에서 Luna가 JSON 숫자 파서 수정 `ba0a10e93e37303af4741024b9fe8461e1ab56ef`을 커밋했습니다. focused pytest 72개, Ruff check/format, strict mypy 통과. 독립 review 두 차례에서 같은 숫자 입력 정밀도·비표준 값 경계가 저장 JSON 경로와 공개 in-memory 경로에 각각 남음을 확인해 사용자 중단 조건을 적용했습니다. review 미통과이므로 구현 커밋은 main에 병합하지 않았고 worktree/branch를 보존합니다. 전체 R2-06은 미체크, 경제 평가는 `not-evaluated`입니다. 재개 시 `ComparisonEnvelope.from_mapping`의 float/NaN 경로를 닫고 fresh 검사·독립 review를 수행해야 합니다. 기록 `docs/development-records/2026-09-23-r2-counterfactual-2817a7d9.md`; 증거와 handoff `/home/kwl/.local/share/jusik/portfolio-audit/20260923-r2-06-2817a7d9/`.
...
- 재시도 통합 (2026-09-23, attempt `9d4a546cee8c4be2a0113239652249e0`): 기준·병합 직전 main `018f7fb96943d4c5dd6566b18c5765841c4ca25d`에서 문서 커밋 `869a69ed1f6ef775c25b3b57e43181d81e8dc877`과 해시 오탈자 수정 `7cbfe7d165518fcda7fa2ac33cff83bc3730cc69`를 local main에 fast-forward 통합했습니다. 최초 통합 gate는 개발 기록의 65자 SHA 오류로 중단했고, 기존 Luna 수정과 독립 review PASS(P1/P2 없음) 후 재개했습니다. 원본·보존 입력 8개, 코드·fixture, 보고서·검증 요약 SHA와 main/worktree identity를 확인했습니다. main Python 3.13.15에서 focused pytest 6개·Ruff check/format·strict mypy·diff 검사가 통과했습니다. 저장 파일럿 CLI는 통합 단계에서 재실행하지 않았습니다.
- 이번 시도의 전체 R2-04 판정은 계속 차단입니다. MDD `26.463097...%`가 20% hard filter를 초과하며 저장 latch 최초 시점·해제 이력과 독립 달력·동일 조건 benchmark·미래 관측 증거가 없습니다. R2-04 체크와 approximate 등급을 유지하며 경제 acceptance를 완료하지 않았습니다.
- 이번 증거·인계: `/home/kwl/.local/share/jusik/portfolio-audit/20260923-r2-04-9d4a546c`의 `integration-preflight.json`, `integration-verification.json`, `REVIEW.md`, `HANDOFF.md`, `integration-evidence.sha256`; 최초 차단 증거는 `blocked-snapshot/`에 해시 검증하여 보존했습니다. 개발 기록 `docs/development-records/2026-09-15-r2-dd-chronology.md`를 갱신했습니다. 필요한 증거와 handoff를 보관·검증한 뒤 이번 소유 worktree `/home/kwl/projects/jusik-r2-dd-chronology-9d4a`와 병합된 브랜치 `feat/r2-dd-chronology-9d4a`만 정상 정리했습니다. 사용자 root `HANDOFF.md`와 다른 worktree는 보존했습니다. 전략·PAPER/live·운영 원장·서비스·설정·시장 입력·remote 변경은 없습니다.
...
- 2026-09-24 R1-05 Stooq 대체 원천 점검: LIME/MDA에 bounded CSV 요청을 보냈으나 두 응답 모두 JavaScript verification challenge HTML만 반환했습니다. raw SHA는 LIME `834d1f405145a5fafc19427540890bdfc91109313eabc677ed2365b37358c68d`, MDA `1b91ef4bfd3ab9c1738601dededbdf7b46fac68c2926befa43fb7245d0a02917`; receipt `/home/kwl/.local/share/jusik/portfolio-audit/20260924-r1-05-receipt/stooq-bounded/`. historical observed_at·원인·coverage 근거가 없어 R1-05/PIT/economic acceptance는 유지합니다. 기록 `docs/development-records/2026-09-24-r1-05-stooq-bounded-receipt.md`.
- 2026-09-24 R1-05 EODHD price fallback: `.env`의 `EODHD_API_KEY`로 LIME/MDA를 bounded 요청했습니다. LIME 59행(첫 관측 2026-07-01, SHA `846ec9df9e5954d72986a2f05186b1915e8f4bb85252f88d793146fc34e2e728`), MDA 135행(첫 관측 2026-03-12, SHA `11af3c72fd437b11765bc1f6a26be4f30aaac31b941ec4ff6eefb9ba5fb36039`)으로 Yahoo와 동일한 부분 이력만 반환됐습니다. publication/observed_at·원인·complete coverage가 없어 366개 누락과 R1-05/PIT/economic acceptance를 유지합니다. receipt `/home/kwl/.local/share/jusik/portfolio-audit/20260924-r1-05-receipt/eodhd-bounded/`; 기록 `docs/development-records/2026-09-24-r1-05-eodhd-price-bounded-receipt.md`.
- 2026-09-24 mandate/runner gate 감사: `_git_ready`·`_roadmap_documents_ready`·`validate_dispatch_gate`를 local main에서 재검증했습니다. policy `investment-roadmap-governance-v1`, dispatch `true`, mandate JSON/digest SHA `22efba4714bc0baf65c56bdfd84dcdee91184a760a13d4d30c5a94486c264ab1`, roadmap SHA `80dc4064b71c6ed129d738e8c73ff0b6f912ced337dc6987dcf8fe8a30bd63e4`가 일치했습니다. 기록 `docs/development-records/2026-09-24-mandate-runner-gate-audit.md`; 경제 승격·실거래는 수행하지 않았습니다.
- 2026-09-24 R2-02 공식 비용 source 보강: Kiwoom 공식 RIA 안내 PDF에서 국내 KRX 유관기관 수수료 `0.0036396%`, NXT `0.0031833%`, 2026-12-31까지 적용 문구를 확인했습니다. 해외 수수료는 별도 홈페이지 안내이고 RIA 자료는 일반 위탁계좌·미국 broker fee·체결/결제 receipt를 확정하지 않으므로 부분 근거로만 보존하며 R2-02 checkbox/경제 acceptance는 유지합니다. 기록 `docs/development-records/2026-09-24-r2-02-kiwoom-official-fee-source.md`; 실거래·PAPER/live·요율 설정 변경 없음.
- 2026-09-24 R2-02 미국 비용 source 충돌: Kiwoom 공식 자료에서 미국 온라인 broker fee `0.25%`는 일치했지만 SEC 비용은 한 자료의 `0.00278%`와 다른 2026 이벤트 페이지의 `0% (이벤트 시작일 기준, 수시변동)`가 충돌했습니다. 계좌/매체/유효기간·적용 대상·체결/결제 receipt가 없어 어느 값도 일반 모델로 승격하지 않았습니다. 기록 `docs/development-records/2026-09-24-r2-02-us-fee-source-conflict.md`; R2-02 checkbox와 경제 acceptance는 유지합니다.
- 2026-09-24 R2-02 SEC Section 31 timeline 보강: SEC FY2025 advisory의 2025-05-14~2026-04-03 `$0/million`과 FY2026 advisory의 2026-04-04 이후 `$20.60/million`(0.00206%)을 canonical pilot 기간에 날짜별로 대사했습니다. 이는 SEC/SRO rate이며 Kiwoom 고객 전가·최소금액·charge/settlement 기준과 다르므로 `sell_tax_rate`에 자동 주입하지 않았습니다. 기록 `docs/development-records/2026-09-24-r2-02-sec-section31-timeline.md`; R2-02 전체 acceptance는 유지합니다.
- 2026-09-24 R2-02 Kiwoom HTS 계산식 조사: 공식 2163/2162 도움말에서 HTS 수수료 `0.25%`, ECN 매수·매도 각각 주당 `$0.003`, 전영업일 최종환율, 체결일 기준 실현손익 계산을 확인했습니다. SEC fee는 한 화면의 `$0.0000218`, 다른 화면의 다른 식/최소 `$0.01`로 충돌하고 비정기 변경 문구가 있어 versioned rate·실제 계좌 receipt 없이는 모델에 주입하지 않습니다. 기록 `docs/development-records/2026-09-24-r2-02-kiwoom-hts-calculation-contract.md`; R2-02 전체 acceptance는 유지합니다.
- 2026-09-24 R1-05 Alpha Vantage 가격 probe: `research_alpha_price_probe`와 6개 회귀 테스트를 추가했습니다. `.env`의 Alpha Vantage 키로 LIME·MDA·AAPL `TIME_SERIES_DAILY`를 각각 bounded 조회했지만 세 응답 모두 HTTP 200 `Information` envelope, 0개 가격 행이었습니다. 키·원문 메시지는 기록하지 않았고 historical/PIT source로 승격하지 않았습니다. 기록 `docs/development-records/2026-09-24-r1-05-provider-alternatives.md`.
