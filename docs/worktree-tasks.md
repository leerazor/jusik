...
- `nasdaq-data-link` 선택 의존성과 공식 Python SDK bounded probe(`jusik.research_nasdaq_data_link`)를 추가했습니다. 실제 `.env` SDK 조회도 `DataLinkError`로 실패해 현재 환경의 edge/bot HTTP 403과 entitlement 판정은 보류했습니다. Alpaca IEX 과거 bar 1건은 인증 성공했으며, 어느 provider도 complete PIT coverage로 승격하지 않았습니다.
