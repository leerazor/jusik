...
- `nasdaq-data-link` 선택 의존성과 공식 Python SDK bounded probe(`jusik.research_nasdaq_data_link`)를 추가했습니다. 일반 `get()`은 `DataLinkError`였지만 사용자가 성공한 `get_table()` 방식으로 `MER/F1`·`compnumber=39102` 1,314행 조회에 성공했습니다. 이는 재무 테이블 접근 증거이며 OHLCV·complete PIT coverage로 승격하지 않았습니다. Alpaca IEX 과거 bar 1건은 인증 성공했습니다.
