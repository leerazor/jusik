...
입력은 `market-performance-metrics-input/v1` JSON이다. 양의 초기 자본, offset이
있는 UTC `timestamp`와 양의 유한 `nav`, `data_grade`(`strict`, `approximate`,
...
- CAGR은 실제 UTC 달력 경과일을 `Decimal(365)`로 나눈 지수로 계산한다.
...
기간이 양수가 아니거나, 수익률 표본이 부족하거나, NAV가 0/음수/비유한이거나,
날짜가 중복·역순이거나, 비용·완전성·무위험률 근거가 없거나, 변동성이 0이면
...
hardlink인 output을 거부한다. 입력 크기와 NAV 개수에도 bounded limit이 있다.
