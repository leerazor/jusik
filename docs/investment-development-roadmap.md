...
- [ ] **R1-02** 기업행사·상장폐지·delisting·중단일을 관측 시점에 맞춰 처리하고 미래 사건을 추가해도 사건 전 선택이 불변인지 확인하는 회귀 fixture를 추가한다.
  - 기술 slice: 연구 기간 이후 이벤트와 시가 이후에 알려진 사건을 추가해도 과거 거래·equity가 변하지 않도록 `known_at <= fill_at` 실행 조건과 회귀 fixture를 통합했습니다. 실제 historical provider receipt·기업행사·상장폐지·중단일 coverage가 없어 전체 checkbox는 미완료로 유지합니다. [개발 기록](development-records/2026-09-20-r1-02-future-event-invariance.md)
...
- [ ] **R1-05** 부분 이력·identity 불일치·상장폐지 심볼은 원인과 coverage로 기록하며 조용히 삭제하지 않는다.
  - public evidence catalog의 SHA·item/source count 무결성 gate를 추가했지만, 이는 provider 전체
    coverage·PIT completeness를 증명하지 않으므로 R1-05 checkbox는 미완료로 유지합니다.
...
- [ ] **R2-02** fee, slippage, `sell_tax_rate`를 KR/US 시장별로 명시하고 매도세·거래세 등 세금 종류와 적용 시점을 검증한다.
  - 공식 source audit 부분 근거: 2026-03-20 시행 `증권거래세법 시행규칙`은 유가증권시장
    ...
    결속하지 못했으므로 R2-02 checkbox와 경제 평가는 유지합니다. Kiwoom 현재 안내의
    국내 KRX/NXT 표기만 별도 source audit에 보존했으며 기존 modeled rate를 치환하지 않습니다.
