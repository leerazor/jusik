...
- latch 시점 보유 수량을 재구성한 뒤 종목별 dataset bar에서 latch 다음 첫 유효 open을 찾고, 그 시점의 전량 sell만 `observed`로 표시한다. open 증거가 없거나 기간 말까지 체결되지 않으면 `not_observed`로 남긴다.
...
동결 파일럿은 독립 MDD·세션별 DD·latch 값과 일치하고 5개 보유 종목의 다음 유효 open 전량 청산을 관측했다. 결과 상태는 `blocked`다. 이 slice에는 별도로 검증된 거래소 calendar, 동일 조건 benchmark, prospective future observation 증거가 없으므로 capability flag만으로 성공을 만들지 않는다. 입력 자료의 `approximate` 등급과 원래 근사 grade는 그대로 유지한다.
