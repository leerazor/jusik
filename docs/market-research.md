...
필수 과거 membership, 일봉, 기업행동, 거래일 달력, 미국 환율 자료가 하나라도 빠지면 수익률이나 거래를 만들지 않고 `검증 불충분`으로 표시한다. 화면의 합성 실행은 인과 흐름과 다음 시가 체결을 확인하기 위한 자료이며 실제 수익률이나 주문이 아니다. 실제 3년 백필과 broker 실행 연결은 이 기능의 범위가 아니다.
