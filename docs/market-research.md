...
이 연구는 한국과 미국을 별도 실행하며 각 실행을 원화 1억원에서 시작한다. 각 거래일의 당시 유효한 주식 membership과 그날 이용 가능한 일봉만으로 거래량 상위 20개를 결정하고, ETF는 제외한다. 역사적 진입 신호는 완료 일봉의 종가가 직전 완료 20세션 고점을 넘고 거래량이 직전 완료 20세션 평균 거래량을 넘을 때 확인하며, 신호 뒤 다음 거래 가능일 시가에 체결한다. 보유 종목은 후보 순위에서 빠졌다는 이유만으로 매도하지 않으며, 완료 종가가 SMA20보다 낮은 날이 2회 연속이면 두 번째 확인 뒤 다음 거래 가능일 시가에 청산한다. 현재 투자자 화면의 오늘 EPS·PER·가치평가 가정은 역사적 신호일에 소급하지 않으며, 역사적 가치 판단에는 해당 시점 이용 가능한 자료가 필요하다.
