...
웹 요청은 준비된 cache를 읽기만 한다. 기간별 자료 준비와 resumable backfill은 별도 CLI 작업이며 KRX·Alpha Vantage 키 또는 자료가 없으면 성공 결과를 만들지 않는다. 파일 원문과 hash, 표본 계약, coverage와 제외 건수를 함께 보존하고 pilot과 final의 source·pool·정규화·누락·기업행동·환율 계약이 다르면 결합하지 않는다.
