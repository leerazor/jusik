...
- gate가 manifest의 schema·경로·paper-only 플래그·source identity·권한을 직접 검증하며,
  변조 시 simulation을 차단합니다. 이 보강을 포함한 focused pytest는 91개입니다.
- manifest 읽기 역시 `O_NOFOLLOW`와 regular-file 검사를 사용하며 symlink 교체 회귀를
  검증합니다.
