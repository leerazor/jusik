...
- `backend/jusik/agent_routing.py`는 stdlib-only `prepare`, `pre`, `post` CLI를 제공합니다. role TOML과 실제 capability probe를 읽고 정확히 다섯 spawn 인자, private manifest, nonce·receipt를 생성합니다.
- `post`는 완결된 parent/child JSONL에서 실제 인자·call output 경로·session parent link·child 첫 assistant receipt·child-owned turn model을 확인합니다. host가 raw initial input을 보존하지 않는 경우 receipt를 delivery evidence로 기록합니다.
...
- `PYTHONPATH=backend backend/.venv/bin/pytest -q backend/tests/test_agent_routing.py` — 통과.
...
- 남은 작업·차단 조건: parent의 full prepared adapter smoke와 통합 검증, 독립 review가 필요합니다.
