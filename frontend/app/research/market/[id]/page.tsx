...
export default async function MarketResearchDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main>
    ...
    {!result ? <section className="panel"><h2>{nullResultMessage.heading}</h2><p>{nullResultMessage.detail}</p></section> : <>
      ...
      <section className="panel limitations"><h2>자료와 한계</h2><ul>{result.limitations.length > 0 ? result.limitations.map((item) => <li key={item}>{item}</li>) : <li>기록된 한계가 없습니다. 추가 자료 완전성은 확인 불가입니다.</li>}</ul><p className="basis">입력 SHA-256 {result.input_hash ?? "없음"} · 정책 SHA-256 {result.policy_hash ?? "없음"}</p></section>
    </>}
  </main>;
}
