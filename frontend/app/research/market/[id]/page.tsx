...
export default async function MarketResearchDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main>
    ...
    <section className="panel research-section" aria-labelledby="historical-rule-title"><div className="section-title simple"><h2 id="historical-rule-title">역사적 신호와 체결 시점</h2><span className="muted">완료 일봉 기준</span></div><p>진입 신호는 완료 종가가 직전 완료 20세션 고점을 넘고 거래량이 직전 완료 20세션 평균 거래량을 넘은 날입니다. 신호 다음 거래 가능일 시가를 진입 체결 시점으로 기록합니다.</p><p>청산 신호는 완료 종가가 SMA20 아래인 날이 2회 연속일 때 확정하고, 다음 거래 가능일 시가를 청산 체결 시점으로 기록합니다.</p><p className="basis">각 역사적 날짜에 이용 가능했던 membership과 일봉만 사용합니다. 현재 투자자 화면의 오늘 EPS·PER·가치평가 가정은 과거 신호일에 소급하지 않으며, 역사적 가치 판단에는 당시 이용 가능했던 자료가 필요합니다.</p></section>
