...
export default async function MarketResearchPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  let approximateReadinessUnavailable = false;
  let runsUnavailable = false;
  try { readiness = await getMarketReadiness(undefined, "strict"); } catch { readinessUnavailable = true; }
  try { readiness = [...readiness, ...(await getMarketReadiness(undefined, "approximate"))]; } catch { approximateReadinessUnavailable = true; }
  try { runs = await getMarketResearchRuns(); } catch { runsUnavailable = true; }
  ...
  return (
    <main>
      ...
      {readinessUnavailable && <section className="notice" role="alert"><h2>시장 연구 준비 상태를 불러오지 못했습니다</h2><p>연구 백엔드 연결을 확인한 뒤 다시 시도하세요.</p></section>}
      {approximateReadinessUnavailable && <section className="notice" role="alert"><h2>무료 근사 자료 준비 상태를 확인하지 못했습니다</h2><p>근사 자료 상태를 확인한 뒤 다시 시도하세요.</p></section>}
      ...
      <section className="research-step" aria-labelledby="runs-title"><div className="step-heading"><p className="eyebrow">STEP 3</p><h2 id="runs-title">저장된 연구</h2></div><div className="panel">{runsUnavailable ? <><h3>저장된 연구를 불러오지 못했습니다</h3><p className="empty-inline">연구 백엔드 연결을 확인한 뒤 다시 시도하세요.</p></> : runs.length === 0 ? <p className="empty-inline">저장된 실행이 없습니다.</p> : <div className="run-list">{runs.map((run) => <Link href={`/research/market/${run.id}`} className="run-row" key={run.id}><span><strong>{stageLabel(run.stage)} · {run.request.market} · {run.request.start_date}–{run.request.end_date}{run.stage === "pilot" && !run.final_promotable ? " · 최종 참조 불가" : ""}</strong><small>{run.created_at}</small></span><span className={`status status-${run.status}`}>{statusLabel(run.status)}</span></Link>)}</div>}</div></section>
    </main>
  );
}
