...
export function StageForm({ pilots }: { pilots: MarketResearchRun[] }) {
  ...
  const [grade, setGrade] = useState<"strict" | "approximate">("strict");
  const [pilotRunId, setPilotRunId] = useState("");
  const marketPilots = useMemo(
    () => pilots.filter((run) => run.request.market === market && run.request.research_grade === grade),
    [grade, market, pilots],
  );
  const selectedPilotId = marketPilots.some((run) => run.id === pilotRunId)
    ? pilotRunId
    : "";
  return (
    <form action={createMarketResearchRun} className="panel research-form">
      ...
      <label>자료 등급<select name="research_grade" value={grade} onChange={(event) => setGrade(event.target.value as "strict" | "approximate")}><option value="strict">엄격한 PIT</option><option value="approximate">무료 근사 자료 · 표본</option></select></label>
      <label>종료일<input name="end_date" type="date" defaultValue="2026-09-14" required /></label>
      <label>참조할 완료 파일럿<select name="pilot_run_id" value={selectedPilotId} onChange={(event) => setPilotRunId(event.target.value)}><option value="">파일럿을 선택하세요 (최종 단계에서 필요)</option>{marketPilots.map((run) => <option value={run.id} key={run.id}>{run.request.market} · {run.request.end_date} · {run.id.slice(0, 8)}</option>)}</select></label>
      <button type="submit">시장 연구 실행</button>
      <p className="basis">파일럿은 종료일 기준 1년, 최종은 3년 전부터 서버가 기간을 계산합니다. 근사 자료는 날짜별 표본과 누락·생존편향 한계를 함께 표시합니다.</p>
    </form>
  );
}
