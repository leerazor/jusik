...
export function ComparisonResults({ study, comparison }: { study: Study; comparison: Comparison }) {
  ...
  const adverse = [
    compareDecimal(comparison.candidate.net_return_pct, comparison.baseline.net_return_pct) === -1 ? "수익이 줄었습니다" : null,
    compareDecimal(comparison.candidate.max_drawdown_pct, comparison.baseline.max_drawdown_pct) === 1 ? "도중에 떨어진 폭이 커졌습니다" : null,
    compareDecimal(comparison.candidate.total_cost_krw, comparison.baseline.total_cost_krw) === 1 ? "거래 비용이 늘었습니다" : null,
  ...
  return <section className={styles.results} aria-label="두 시험에서 관측한 결과">
    ...
    <div className={styles.interpretation}><strong>숫자를 읽고 가져갈 결론</strong><p>{narrative?.reading.result ?? "이 자료에 맞는 연구 설명을 확인하지 못했습니다. 표시된 수치만으로 더 좋은 투자법을 정하지 마세요."}</p>{adverse.length > 0 && <p>이 비교에서 바꾼 시험은 {adverse.join(". ")}. 이 부담도 함께 보세요.</p>}</div>
