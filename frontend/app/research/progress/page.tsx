  ...
  return (
    <section className={styles.featuredPanel} aria-labelledby="featured-title">
      <div className={styles.sectionHeading}>
        <div><p className={styles.kicker}>대표 비교 · {study.title}</p><h2 id="featured-title">같은 기간·같은 비용 조건으로 보기</h2><p className={styles.muted}>{comparison.period_start}–{comparison.period_end} · 비용 {comparison.cost_multiplier}배 · {cashLabel} · 낙폭 기준 {comparison.drawdown_basis === "close_nav" ? "종가 평가액" : "전체 관측 평가액"}</p></div>
...
function ProgressPurpose() {
  return <section className={styles.purposeSection} aria-labelledby="progress-purpose-title"><p className={styles.kicker}>HOW TO READ THIS PAGE</p><h2 id="progress-purpose-title">먼저 질문과 조건, 다음으로 양쪽 수치를 읽습니다</h2><p>각 연구의 비교 표는 같은 종목·기간·비용 조건에서 기존 방식과 시험 방식을 보여줍니다. 누적 수익률·현금·낙폭·거래·비용을 함께 보고, 과거 비교를 채택이나 실거래 적합성으로 해석하지 않습니다.</p></section>;
}
