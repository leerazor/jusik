...
import { getStudyNarrative } from "@/lib/research-narrative";
...
function ComparisonPanel({ study, comparison }: { study: Study; comparison: Comparison }) {
  ...
  return (
    <section className={styles.featuredPanel} aria-labelledby="featured-title">
      ...
      <p className={styles.metricExplainer}>수익률은 이 비교 기간 전체의 누적 변화이며 연환산이 아닙니다. 최대 낙폭은 평가액 고점 뒤 가장 크게 줄어든 폭입니다. 현금 비중은 기간 중 현금의 {comparison.cash_statistic === "mean" ? "평균" : "중앙값"}이고, 거래일은 실제 주문 횟수가 아니라 거래가 발생한 날짜 수입니다. 비용과 회전율이 함께 늘면 수익률만으로 개선이라 할 수 없습니다. {comparison.drawdown_basis === "close_nav" ? "종가 NAV만으로 계산한 낙폭은 모든 관측 시점의 목표 충족을 증명하지 않습니다." : "모든 관측 NAV 기준이라도 이 과거 비교의 범위 안에서만 읽습니다."}</p>
...
function StudiesSection({ progress }: { progress: ResearchProgress }) {
  ...
  return <section className={styles.studiesSection} aria-labelledby="studies-title"><div className={styles.sectionHeading}><div><p className={styles.kicker}>공개 연구 4개 · 비교 24개</p><h2 id="studies-title">연구 질문과 결과</h2><p className={styles.muted}>각 연구의 질문과 고정 조건을 먼저 확인한 뒤, 세부 비교를 펼쳐 기준선과 후보를 읽습니다.</p></div><Link className={styles.textLink} href="/research/history">전체 연구 이력 보기</Link></div>{progress.research.availability !== "available" ? <div className={styles.unavailableBox}>검증된 연구 카탈로그를 표시할 수 없습니다.</div> : studies.length === 0 ? <div className={styles.emptyPanel}>공개된 연구가 아직 없습니다.</div> : <div className={styles.studyGrid}>{studies.map((study) => { const narrative = getStudyNarrative(study); return <article className={styles.studyCard} id={study.id} key={study.id}><div className={styles.studyCardHeading}><div><span className={styles.cardEyebrow}>연구 질문</span><h3>{study.title}</h3></div><Link className={styles.reportLink} href={`/research/history/download/${study.report_artifact_sha256}`}>보고서</Link></div>{narrative ? <><p className={styles.studyQuestion}>{narrative.question}</p><div className={styles.narrativeGrid}><div><strong>관련 목표</strong><p>{narrative.relatedGoals.join(" · ")}</p></div><div><strong>기존 방식</strong><p>{narrative.baselineRules.join(" · ")}</p></div><div><strong>시험 방식</strong><p>{narrative.candidateRules.join(" · ")}</p></div><div><strong>고정 조건</strong><p>{narrative.fixedConditions.join(" · ")}</p></div></div><p className={styles.decisionNote}><strong>결과/한계:</strong> {narrative.conclusion} {narrative.limitations.join(" ")} <strong>결정 기록 없음.</strong></p></> : <div className={styles.unavailableBox}>연구 설명을 확인할 수 없습니다. API가 제공한 제목·수치·보고서 링크는 유지합니다.</div>}<p className={styles.studyMetaLine}>공개 {formatDateTime(study.published_at)} · {study.universe_symbols.length}종목 · 과거 가격만 · 배당·세금 제외 · source/result 해시 {narrative ? "확인됨" : "불일치"}</p><div className={styles.comparisonDetails}>{study.comparisons.length === 0 ? <p className={styles.emptyInline}>비교 조건 없음</p> : study.comparisons.map((comparison) => <details key={comparison.id}><summary><span>{comparison.period_start}–{comparison.period_end} · 비용 {comparison.cost_multiplier}배 · 현금 {comparison.cash_statistic === "mean" ? "평균" : "중앙값"}</span><strong>{comparison.baseline.label} ↔ {comparison.candidate.label}</strong></summary><div className={styles.tableWrap}><table><caption>{comparison.id} 비교</caption><thead><tr><th scope="col">지표</th><th scope="col">기준선 · {comparison.baseline.label}</th><th scope="col">후보 · {comparison.candidate.label}</th></tr></thead><tbody><tr><th scope="row">순수익률</th><td>{formatPercent(comparison.baseline.net_return_pct)}</td><td>{formatPercent(comparison.candidate.net_return_pct)}</td></tr><tr><th scope="row">현금 비중 ({comparison.cash_statistic === "mean" ? "평균" : "중앙값"})</th><td>{formatPercent(comparison.baseline.cash_pct)}</td><td>{formatPercent(comparison.candidate.cash_pct)}</td></tr><tr><th scope="row">최대 낙폭 ({comparison.drawdown_basis === "close_nav" ? "종가 NAV" : "전체 관측 NAV"})</th><td>{formatPercent(comparison.baseline.max_drawdown_pct)}</td><td>{formatPercent(comparison.candidate.max_drawdown_pct)}</td></tr><tr><th scope="row">최대 레버리지</th><td>{formatPercent(comparison.baseline.max_leverage_pct)}</td><td>{formatPercent(comparison.candidate.max_leverage_pct)}</td></tr><tr><th scope="row">연환산 회전율</th><td>{formatPercent(comparison.baseline.annual_turnover_pct)}</td><td>{formatPercent(comparison.candidate.annual_turnover_pct)}</td></tr><tr><th scope="row">총 비용</th><td>{formatKrw(comparison.baseline.total_cost_krw)}</td><td>{formatKrw(comparison.candidate.total_cost_krw)}</td></tr><tr><th scope="row">거래일</th><td>{formatCount(comparison.baseline.trade_days)}일</td><td>{formatCount(comparison.candidate.trade_days)}일</td></tr></tbody></table></div><p className={styles.comparisonBasis}>기간 {comparison.period_start}–{comparison.period_end} · 비용 {comparison.cost_multiplier}배 · 현금은 UTC 하루 마지막 NAV의 {comparison.cash_statistic === "mean" ? "평균" : "중앙값"} · 낙폭은 {comparison.drawdown_basis === "close_nav" ? "종가 NAV" : "모든 관측 NAV"}</p></details>)}</div></article>; })}</div>}</section>;
}
  ...
  return (
    <main className={styles.progressMain}>
      ...
      <section className={styles.hero}>
        <div><p className={styles.kicker}>RESEARCH PROGRESS</p><h1>성과 비교</h1><p className={styles.heroCopy}>두 방식의 실제 수치와 연구별 조건을 비교합니다. 완료율이나 미래 성과를 추정하지 않습니다.</p></div>
      ...
      {!progress ? <section className={styles.unavailableBox} role="alert"><h2>진행 현황을 불러올 수 없습니다</h2><p>연구 진행 API가 아직 연결되지 않았거나 응답을 검증하지 못했습니다. 자동 실행 중이나 정상 대기로 해석하지 않습니다.</p></section> : <><ProgressPurpose /><StudiesSection progress={progress} /><FeaturedSection progress={progress} /><details className={styles.operationsDetails}><summary>운영 상세 · 자동 실행기와 작업 대기열</summary><RunnerOverview progress={progress} /><QueueSection progress={progress} /></details><footer className={styles.footer}>모든 연구는 과거 가격 데이터만 사용하며 배당과 세금을 포함하지 않습니다. 회고용으로 재사용된 데이터이고 point-in-time 검증이 아니므로 미래 성과를 입증하지 않습니다. · <Link href="/research/history">연구 이력과 보고서</Link></footer></>}
    </main>
  );
}
