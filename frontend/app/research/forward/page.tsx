...
export default async function ForwardResearchPage() {
  ...
  try {
    const results = await Promise.allSettled([getForwardStatus(), getForwardLedger()]);
  ...
  return (
    <main>
      ...
      {statusError || ledgerError ? (
        ...
        <>
          <section className="panel forward-intro-panel"><div className="section-title simple"><h2>관찰 대상과 현재 기록</h2><span className="status">PAPER 10% 고정</span></div><p>이 정책은 연구 목표 20%와 별도이고, 비교 후보를 자동 채택하지 않습니다. 다음 정기 시각은 운용 정책의 예정 시각이며 연구 평가 완료 시각이 아닙니다.</p><dl className="metric-list"><div><dt>정책 hash</dt><dd title={status.session.policy_hash}>{status.session.policy_hash.slice(0, 16)}…</dd></div><div><dt>활성화 시각</dt><dd>{kst(status.session.activated_at)}</dd></div><div><dt>다음 정기 시각</dt><dd>{kst(status.session.next_due_at)}</dd></div><div><dt>평가 종료 조건</dt><dd>자료에 없음</dd></div><div><dt>기록 수</dt><dd>체결 {ledger.fills.length}건 · 시세 {status.feed.items.length}종목 · 기업행동 {status.corporate_actions.length}건</dd></div><div><dt>비교 후보</dt><dd>연구 결과에서 별도 확인</dd></div></dl></section>
