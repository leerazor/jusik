...
def test_telegram_requires_explicit_opt_in_and_complete_secrets() -> None:
    ...
    with pytest.raises(ValidationError):
        settings(telegram_bot_token="token", telegram_chat_id=None)
    configured = settings(
        telegram_enabled=True,
        telegram_bot_token="private-bot-value",
        telegram_chat_id="private-chat-value",
    )
    assert configured.telegram_enabled is True
    dumped = repr(configured)
    assert "private-bot-value" not in dumped
    assert "private-chat-value" not in dumped
