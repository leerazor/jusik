...
def covariance_proxy(weights: dict[str, Decimal], returns: dict[str, list[Decimal]]) -> Decimal:
    ...
    mean = {
        symbol: sum(returns[symbol], Decimal(0)) / Decimal(length)
        for symbol in symbols
    }
    variance = Decimal(0)
    for index in range(length):
        portfolio_return = sum(
            (weights[symbol] * (returns[symbol][index] - mean[symbol]) for symbol in symbols),
            Decimal(0),
        )
...
def diagnose_simulation(simulation: dict[str, Any], *, gross_cap: Decimal = Decimal("0.95"),
                        volatility_target: Decimal = Decimal("0.30")) -> dict[str, Any]:
    ...
    for event in events:
        if event.get("kind") == "volatility_scale":
            ...
            scales[str(event["at"])] = min(Decimal(1), volatility_target / proxy) if proxy else Decimal(1)
    ...
    for at in sorted(set(targets) | set(scales)):
        ...
        rows.append({"decided_at": at, "pre_vol_gross": pre,
                     "post_vol_gross": post, "volatility_scale": scale,
                     "unallocated_target": max(Decimal(0), gross_cap - post),
                     "recorded_target_gross": post,
                     "eligible_count": sum(value > 0 for value in targets.get(at, {}).values()),
                     "excluded_reasons": {}, "pre_vol_source": "inferred_from_saved_target"})
    counts = Counter(str(event.get("kind")) for event in events)
    return {
        "stages_are_overlapping_and_not_additive_cash_shares": True,
        "decision_rows": rows,
        "event_counts": dict(sorted(counts.items())),
        "reentry_latches": [{"at": event.get("at"), "kind": event.get("kind"),
                             "detail": event.get("detail")}
                            for event in events if str(event.get("kind", "")).startswith("reentry")
                            or event.get("kind") in {"recovery_confirmation", "recovery_reset"}],
        "cap_events": [{"at": event.get("at"), "kind": event.get("kind"),
                        "detail": event.get("detail")}
                       for event in events if event.get("kind") == "cap_constraint_deferred"],
        "gross_cap": gross_cap,
        "volatility_target": volatility_target,
    }


def recompute_causal_stages(source_path: Path, engine_path: Path,
                            simulation: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    from jusik.research_portfolio_models import PortfolioCandidate, PortfolioConfig, PortfolioInput
    parsed = PortfolioInput.model_validate(source)
    config = PortfolioConfig(initial_cash_krw=Decimal("100000000"), gross_cap=Decimal("0.95"),
                             volatility_target=Decimal("0.30"), low_turnover_weeks=4,
                             low_turnover_band=Decimal("0.02"), drawdown_limit=Decimal("0.10"),
                             symbol_cap=Decimal("0.20"), leveraged_etf_cap=Decimal("0.20"))
    candidate = PortfolioCandidate(id="diagnostic", method="inverse_volatility", gate="fx_vix")
    dates = sorted({str(row["decided_at"]) for row in simulation.get("weekly_targets", [])})
    ...
    for value in dates:
        ...
        scale, proxy = module.volatility_scale(parsed, module._instrument_data(parsed), weights, at, config)
        if scale is None:
            raise ValueError(f"missing causal volatility data at {value}")
        output[value] = {"pre_vol_gross": sum(weights.values(), Decimal(0)),
                         "post_vol_gross": sum(weights.values(), Decimal(0)) * scale,
                         "volatility_scale": scale, "volatility_proxy": proxy,
                         "eligible_count": len(weights), "blocked_reason": blocked,
...
def run_diagnostic(prior_audit: Path, output_dir: Path) -> dict[str, Any]:
    ...
    source_path, engine_path = Path(request["source_path"]), Path(request["engine_path"])
    if sha256(source_path) != request["source_sha256"] or sha256(engine_path) != request["engine_sha256"]:
    ...
    for path in files:
        ...
        entries.append({"period": period, "simulation_path": str(path),
                        "simulation_sha256": sha256(path), "observation_path": str(observation),
                        "observation_sha256": sha256(observation),
                        "diagnostic": diagnose_simulation(simulation),
                        "causal_stages": recompute_causal_stages(source_path, engine_path, simulation)})
    result = {"run_id": "portfolio-residual-cash-risk-proxy-v1",
              "read_only": True, "historical_simulations_executed": 0,
              "prior_audit": str(prior_audit), "entries": entries,
              "provenance": {"prior_audit_sha256": sha256(prior_audit / "experiment" / "results.json"),
                             "module_sha256": sha256(Path(__file__))}}
    (output_dir / "diagnostic.json").write_text(json.dumps(result, ensure_ascii=False,
