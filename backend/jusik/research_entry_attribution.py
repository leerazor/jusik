...
def _validate_frozen_metadata(
    ...
    if (
        ...
        or row.get("start") != sim.period_start.isoformat()
        or row.get("end") != sim.period_end.isoformat()
    ):
    ...
    if row.get("complete") is not True:
        raise ValueError(f"frozen evaluation is incomplete: {row['artifact']}")
...
def attribute_simulation(
    ...
    with localcontext() as context:
        ...
        position_symbols = [position.symbol for position in sim.positions]
