...
PRIOR_MANIFEST_SHA256 = (
    "332ddf7728a8e92e720a92541fb0b3d778fd9130b61a9abdc5ed612b43ce78a2"
)
PRIOR_RESULTS_SHA256 = (
    "1487e612d747cb960d20065ac3a6ce780d07136f40d80af67600c14682d1ff91"
)
...
def _config(base: PortfolioConfig, cadence: int, cost: int) -> PortfolioConfig:
    ...
    if isinstance(cost, bool) or cost not in FULL_COSTS:
        raise ValueError(
            "cost multiplier must be one of the preregistered values 1 or 3"
        )
...
def _load_contract(
    ...
    if set(prior_rows) != expected:
        raise ValueError(
            "prior volatility audit must provide exactly 16 variant controls"
        )
    for name, row in prior_rows.items():
        verify_hashes(
            {prior_audit / "simulations" / cast(str, name): row.get("sha256", "")}
        )
    return (
        prereg,
...
def _runtime_hashes(
    ...
    prior_manifest = _strict_json(prior_audit / "hash-manifest.json")
    ...
    for name, digest in {
        **prereg.get("core_hashes", {}),
        **prereg.get("imported_helper_hashes", {}),
    }.items():
...
def _execute(
    ...
    try:
        ...
        output_prereg = {
            ...
            "validated_configs": {
                f"c{cadence}-cost{cost}": _config(base, cadence, cost).model_dump(
                    mode="json"
                )
                for _arm, cadence in ARMS
                for cost in FULL_COSTS
            },
            "candidate": candidate.model_dump(mode="json"),
            "policy": "low_turnover_combined",
            "mandate": _strict_json(mandate),
            "mandate_sha256": MANDATE_SHA256,
        }
        _write_exclusive(output_dir / "preregistration.json", output_prereg)
        runtime_hashes = _runtime_hashes(
            prior_audit, prereg, engine_source, original, variant, observer, mandate
        )
            ...
            for period in periods:
                for cost in FULL_COSTS:
                    ...
                    if clock() - started >= DEADLINE_SECONDS:
                        raise TimeoutError(
                            "evaluation deadline exceeded after simulation"
                        )
                    verify_hashes(runtime_hashes)
                    ...
                    if arm == "control":
                        ...
                        prior = next(
                            row
                            for row in prior_results["evaluations"]
                            if row.get("artifact") == prior_name
                        )
                        _verify_exact_replay(
                            payload,
                            prior_audit / "simulations" / prior_name,
                            prior["sha256"],
                        )
                ...
                pairs.append(
                    {
                        ...
                        "cost_multiplier": cost,
                        "delta": {
                            "daily_mean_cash_krw": str(
                                Decimal(vd["daily_mean_cash_krw"])
                                - Decimal(cd["daily_mean_cash_krw"])
                            ),
                            "daily_mean_cash_pct": str(
                                Decimal(vd["daily_mean_cash_pct"])
                                - Decimal(cd["daily_mean_cash_pct"])
                            ),
                            "total_return_pct": str(
                                Decimal(vm["total_return_pct"])
                                - Decimal(cm["total_return_pct"])
                            ),
                            "turnover_pct": str(
                                Decimal(vm["turnover_pct"])
                                - Decimal(cm["turnover_pct"])
                            ),
                            "trade_count": vm["trade_count"] - cm["trade_count"],
                            "transaction_cost_krw": str(
                                Decimal(vm["transaction_cost_krw"])
                                - Decimal(cm["transaction_cost_krw"])
                            ),
                            "fx_cost_krw": str(
                                Decimal(vm["fx_cost_krw"]) - Decimal(cm["fx_cost_krw"])
                            ),
                            "total_cost_krw": str(
                                Decimal(vm["transaction_cost_krw"])
                                + Decimal(vm["fx_cost_krw"])
                                - Decimal(cm["transaction_cost_krw"])
                                - Decimal(cm["fx_cost_krw"])
                            ),
                            "observer_mdd_pct": str(
                                Decimal(variant["global_drawdown_pct"])
                                - Decimal(control["global_drawdown_pct"])
                            ),
                            "gross_cap_exceed_count": len(vc["excess"]["gross"])
                            - len(cc["excess"]["gross"]),
                            "symbol_cap_exceed_count": len(vc["excess"]["symbol"])
                            - len(cc["excess"]["symbol"]),
                            "leveraged_cap_exceed_count": len(vc["excess"]["leveraged"])
                            - len(cc["excess"]["leveraged"]),
                        },
                    }
                )
        result = {
        ...
        if output_dir.is_dir() and not (output_dir / "failure.json").exists():
            ...
            _write_exclusive(
                output_dir / "failure.json",
                {
                    ...
                    "missing": sorted(_expected_names() - completed),
                    "restart_requires": [
                        "explicit new attempt approval",
                        "new empty output directory",
                        "same frozen source/helper/model/preregistration hashes",
