...
`/investor`에서 한국·미국 후보를 발견하고 직접 조회한 현재 사실을 확인한 뒤 투자 근거(thesis)를 저장할 수 있습니다. 후보 순위는 KIS 첫 페이지의 bounded discovery이며 전체 시장이나 저평가·품질을 보장하지 않습니다. 상세 자료에는 출처, 조회 시각, 재무 기간, 누락 이유가 함께 표시됩니다. 조회 시각은 거래소 시세 시각이나 공시 시각과 다릅니다.

가치 진입 계산은 사용자가 입력한 정규화 EPS와 목표 PER 범위로 `EPS × 목표 PER`를 계산하는 가정 가격 범위입니다. 안전마진과 서면 근거 없이는 평가하지 않습니다. 조정 완료 일봉이 있을 때만 초기 추세 돌파와 이틀 연속 SMA20 하회를 관찰합니다. ETF와 타입 미확인 종목은 기업 가치평가에서 제외합니다. 저장 기록은 개인 연구 기록이며 체결·보유 상태나 자동 주문·알림이 아닙니다. 자세한 규칙은 [`docs/investor-workflow.md`](docs/investor-workflow.md)를 참고하세요.
...
새 개발 작업은 [구조 안내](docs/architecture.md)와 [개발 기록 기준](docs/development-records.md)부터 확인합니다. 진행·차단 작업은 [작업 등록부](docs/worktree-tasks.md), 워크트리 통합 절차는 [워크트리 운영 문서](docs/worktree-workflow.md)에 있습니다.
